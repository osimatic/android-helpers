 ...
 * See {@link R.styleable#DurationPicker_textAppearanceDisplay},
 * {@link R.styleable#DurationPicker_timeUnits},
 * {@link R.styleable#DurationPicker_textAppearanceUnit},
 * {@link R.styleable#DurationPicker_textAppearanceButton},
 * {@link R.styleable#DurationPicker_backspaceIcon},
 * {@link R.styleable#DurationPicker_clearIcon},
 * {@link R.styleable#DurationPicker_separatorColor},
 * {@link R.styleable#DurationPicker_durationDisplayBackground},
 * {@link R.styleable#DurationPicker_numPadButtonPadding}
 */
public class DurationPicker extends FrameLayout {
	...
	public DurationPicker(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.durationPickerStyle);
	}

	public DurationPicker(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// apply style
		//
		final TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.DurationPicker, defStyleAttr, 0);
		try {
			...
			applyBackgroundColor(attributes, R.styleable.DurationPicker_separatorColor, separatorView);
			applyBackgroundColor(attributes, R.styleable.DurationPicker_durationDisplayBackground, displayRow);

			applyUnits(attributes, R.styleable.DurationPicker_timeUnits);
		} finally {
			attributes.recycle();
		}
	...
	private static class TimeDurationString {
		...
		private void setMaxDigits(int timeUnits) {
			if (timeUnits == DurationPicker.HH_MM_SS)
				maxDigits = 6;
			else
				maxDigits = 4;
		...
		public long getDuration() {
			...
			return DurationUtils.durationOf(hours, minutes, seconds);
		}

		public void setDuration(long duration) {
			this.duration = duration;
			setDuration(
					DurationUtils.hoursOf(duration),
					timeUnits == MM_SS ? DurationUtils.minutesOf(duration) : DurationUtils.minutesInHourOf(duration),
					DurationUtils.secondsInMinuteOf(duration));
		}
